...
//   useFocusEffect(
//     useCallback(() => {
//       console.log("[Banner Focus] Home focused → refreshing banners");
//       fetchPromotionalBanners();

//       const interval = setInterval(() => {
//         console.log("[Banner Poll] 30s polling refresh");
//         fetchPromotionalBanners();
//       }, 30_000);
...
//             const bannerId = payload.new?.promotional_banner_id;
...
//             if (newStatus === "COMPLETED" && oldStatus !== "COMPLETED" && bannerId) {
//               console.log("[Banner Realtime] Banner booking completed → refreshing banners...");
//               fetchPromotionalBanners();
//             }
//           }        )
//         .subscribe((status) => {
...
//         popupSliderRef.current?.scrollToIndex({ index: next, animated: true });
//         return next;
//       });
//     }, 1500); // 3 seconds per slide

//     return () => clearInterval(interval);
//   }, [showPopup, popupType, appPopups.length]);
...
//     // Also make sure the user has never completed a paid booking
//     const { count: totalPaidCount } = await supabase
//       .from("bookings")
//       .select("id", { count: "exact", head: true })
//       .eq("user_id", session.user.id)
