...
const fetchPromotionalBanners = useCallback(async () => {
  ...
  if (data) {
    try {
      ...
      console.log("=== BANNER ELIGIBILITY CHECK ===");
      console.log("User ID:", userId);
      console.log("Completed Service Count:", completedServiceCount);
      console.log("Customer Pincode:", customerPincode);
      console.log("Resolved Hub IDs:", customerHubIds);
      console.log("Current Service ID:", currentServiceId);
      ...
      const eligibleBanners = data.filter(banner => {
        ...
        if (!banner.customer_type || banner.customer_type === "everyone") {
          ...
          customerEligible = completedServiceCount === 0;
        } else if (banner.customer_type === "existing") {
          customerEligible = completedServiceCount > 0;
        }
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      ...
      console.log("[Banner Realtime] Subscribing for user:", userId);

      subscription = supabase
        .channel(`banner-bookings-${userId}`)
        .on(
          "postgres_changes",
          {
            event: "UPDATE",
          ...
          (payload: any) => {
            const newStatus = String(payload.new?.work_status || "").toUpperCase();
            const oldStatus = String(payload.old?.work_status || "").toUpperCase();

            console.log(
              "[Banner Realtime] Booking updated:",
              payload.new?.id,
              `${oldStatus} → ${newStatus}`
            );

            if (newStatus === "COMPLETED" && oldStatus !== "COMPLETED") {
              console.log("[Banner Realtime] Service completed → refreshing banners...");
    ...
    return () => {
      isCancelled = true;
      if (subscription) {
        console.log("[Banner Realtime] Unsubscribing");
        supabase.removeChannel(subscription);
      }
    };
  }, [fetchPromotionalBanners]);
  // ⬆️⬆️⬆️ END OF REALTIME USEEFFECT ⬆️⬆️⬆️
  const onRefresh = async () => {
    setRefreshing(true);
    await Promise.all([fetchMainCategories(), fetchServices(), fetchHeroBanners(), fetchPromotionalBanners(), fetchPopups()]);
    setRefreshing(false);
...
//     // ✅ Popup state
//   const POPUP_WIDTH = Math.min(width - 48, 360);
//   // AnimatedGradientBorder adds 2px margin on each side of inner content,
//   // so the FlatList item width must match this inner width for snapping to work.
//   const POPUP_BORDER_WIDTH = 2;
//   const POPUP_INNER_WIDTH = POPUP_WIDTH - POPUP_BORDER_WIDTH * 2;
...
//   const [activeOffers, setActiveOffers] = useState<{ service_type: string; title: string; offer_percentage: number; description: string | null }[]>([]);
//   const [showPopup, setShowPopup] = useState(false);
//   // Tracks whether the user is actively dragging the popup slider → pauses auto-slide
//   const [isPopupUserSwiping, setIsPopupUserSwiping] = useState(false);
//   // Queued second popup — shows after Popup 1 closes
//   const [pendingPopup2, setPendingPopup2] = useState<
//     "WELCOME" | "SIGNUP_OFFER" | null
//   >(null);
//   const { showToast } = useNotification();
...
//         // ✅ Queue this popup. It will fire automatically once Popup 1 closes.
//         setPendingPopup2("WELCOME");
...
//         // ✅ Queue this popup. WELCOME takes priority if both are queued.
//         setPendingPopup2((prev) => prev ?? "SIGNUP_OFFER");
...
//   // ✅ Auto-slide Popup 1 every 3 seconds.
//   // Pauses while the user is dragging it.
//   useEffect(() => {
//     if (popupType !== "APP_POPUP") return;
//     if (appPopups.length <= 1) return;
//     if (!showPopup) return;
//     if (isPopupUserSwiping) return;

//     const interval = setInterval(() => {
//       setPopupIndex((prev) => {
//         const next = (prev + 1) % appPopups.length;
...
//     return () => clearInterval(interval);
//   }, [popupType, appPopups.length, showPopup, isPopupUserSwiping]);

//   // ✅ Once Popup 1 closes, show the queued Popup 2 (Welcome / Signup Offer).
//   useEffect(() => {
//     if (showPopup) return;              // Popup 1 still visible
//     if (!pendingPopup2) return;         // nothing queued
//     if (showWelcomePopup) return;       // already showing
//     if (showSignupOfferPopup) return;   // already showing

//     // Small delay so Popup 1's close animation completes cleanly
//     const t = setTimeout(() => {
//       if (pendingPopup2 === "WELCOME") {
//         setShowWelcomePopup(true);
//       } else if (pendingPopup2 === "SIGNUP_OFFER") {
//         setShowSignupOfferPopup(true);
//       }
//       setPendingPopup2(null);
//     }, 350);
...
//               style={[popupStyles.container, { backgroundColor: theme.background }]}
//             >              <Pressable style={popupStyles.closeBtn} onPress={() => {
//                 if (popupType === "APP_POPUP" && activeOffers.length > 0) {
//                   setPopupType("OFFERS");
//                 } else {
...
//                     scrollEnabled={true}
//                     showsHorizontalScrollIndicator={false}
//                     snapToInterval={POPUP_INNER_WIDTH}
//                     snapToAlignment="start"
//                     decelerationRate="fast"
//                     disableIntervalMomentum
//                     bounces={false}
//                     keyExtractor={(_, i) => `popup-${i}`}
//                     getItemLayout={(_, index) => ({
//                       length: POPUP_INNER_WIDTH,
//                       offset: POPUP_INNER_WIDTH * index,
//                       index,
//                     })}
//                     onScrollBeginDrag={() => setIsPopupUserSwiping(true)}
//                     onScrollEndDrag={() => setIsPopupUserSwiping(false)}
//                     onMomentumScrollEnd={(e) => {
//                       setIsPopupUserSwiping(false);
//                       const idx = Math.round(
//                         e.nativeEvent.contentOffset.x / POPUP_INNER_WIDTH
//                       );
//                       if (idx >= 0 && idx < appPopups.length) {
//                         setPopupIndex(idx);
//                       }
//                     }}
...
//                     }}
//                     renderItem={({ item }) => (
//                       <View style={{ width: POPUP_INNER_WIDTH }}>
//                         {item.image_url ? (
//                           <Image
...
//                         <View style={popupStyles.appContent}>
//                           <Text
//                             style={[popupStyles.appTitle, { color: theme.text }]}
//                           >
//                             {item.title}
//                           </Text>
//                           {item.description && (
