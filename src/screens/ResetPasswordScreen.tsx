...
export default function ResetPasswordScreen() {
    ...
    const handleSendResetEmail = async () => {
        ...
        setLoading(true);
        try {
            const redirectUrl = Linking.createURL("reset-password", { scheme: "theneatifyteam" });
            console.log("PASSWORD RESET REDIRECT URL:", redirectUrl);
            const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), { redirectTo: redirectUrl });
            if (error) {
                console.log("RESET PASSWORD ERROR:", error);
                console.log("Details -> message:", error.message, "status:", error.status, "code:", (error as any).code);
                throw error;
            }
